...
function _JS_Microphone_End(deviceIndex) {
 ...
 if (ctx && ctx.stream) {
  ctx.stream.userMediaStream.getTracks().forEach(function(track) {
   track.stop();
  });
...
function _JS_Microphone_Start(deviceIndex, bufferInstance, samplesPerUpdate) {
 ...
 var handleStream = function(userMediaStream) {
  ...
  stream.gainNode = new GainNode(ctx.audioContext, {
   gain: 0
  });
...
function _StopRecordingAndSendToAPI() {
 if (this.mediaRecorder) {
  ...
  this.mediaRecorder.addEventListener("stop", async () => {
   ...
   var response = await fetch("https://shloka.herokuapp.com/process_wav_js", {
    method: "POST",
    body: formData
   });
   var data = await response.json();
   var returnStr = JSON.stringify(data);
   SendMessage("yourButton", "ReceiveSimilarityResult", returnStr);
